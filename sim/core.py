...
def lock_enemy(vals, instinct):
    vals=list(vals)
    if len(vals)<=2 or instinct=="BOTH":
        return vals[:2]
    if instinct=="STRONGEST":
...
def item_resource_cost(action):
    ...
    typ=action[0]
    return {"bump":1,"flip":1.5,"copy":1.5,"transmute":2.5,"set4":2.0,"carbon_copy":2.2}.get(typ,1.0)

def self_variants(dice, fixed_idx, resources, player):
    out=[(list(dice),None)]
    allowed=[i for i in range(4) if i!=fixed_idx]
    if resources["bump"]>0:
        for i in allowed:
            for delta in (-1,1):
    ...
    if "counterfeit_seal" in player.contraband:
        for i in allowed:
            if dice[i]!=4:
                nv=list(dice);nv[i]=4;out.append((nv,("set4",i)))
...
def enemy_variants(enemy_locked, resources, player, boss_rule=None):
    ...
    if "wire_cutter" in player.contraband:
        nv=list(enemy_locked);i=max(range(len(nv)),key=lambda j:nv[j]);nv[i]=max(1,nv[i]-1);out.append((nv,("wire_cutter",i)))
    return out
...
def evaluate_candidate(player, enemy, dice, pair, enemy_locked, self_action, enemy_action, armor_available, best_spoils, caliper_available, boss_rule=None):
    raw=sum(dice[i] for i in pair)
    fight=raw+player_fight_bonus(player,dice,pair)
    spoils_vals=[dice[i] for i in range(4) if i not in pair]
    base_spoils=sum(spoils_vals)
    enemy_total=sum(enemy_locked)
    rule=enemy.get("rule")
    manipulation=self_action is not None or enemy_action is not None
    self_manip=self_action is not None
    ...
    margin=fight-enemy_total
    damage=incoming=0;spoils_score=None;caliper_trigger=False
    if margin>0:
        damage=margin
        ...
        if armor_available:
            ...
            if armor in ("work_apron","old_field_coat"): incoming=max(0,incoming-1)
...
def choose_candidate(cands, policy, player, best_spoils):
    ...
    def score(c):
        ...
        if margin>0:
            oldband=spoils_band(best_spoils or 2);newband=spoils_band(c["spoils_score"] or 2);gain=max(0,newband-oldband)
            greed_weight=25 if hp_ratio>0.65 else 12 if hp_ratio>0.4 else 4
...
def consume_action(player, action, resources):
    ...
    elif typ=="set4": player.contraband.remove("counterfeit_seal")
...
def combat_mode(player, enemy_key, policy, rng, boss=False, trace=False, spoils_mode="best"):
    ...
    while enemy["hp"]>0 and player.hp>0 and rounds<40:
        ...
        if choice["margin"]<0 and "redacted_slip" in player.contraband and policy!="greedy":
            nd,used=try_redacted_reroll(player,dice,fixed_idx,rng)
            if used:
                ...
                if alt["margin"]>=choice["margin"]:choice=alt
        ...
        if choice["incoming"]>0:
            ...
            if armor_available and player.gear["armor"] in ("work_apron","old_field_coat","proof_vest","inside_out_lining"):armor_available=False
