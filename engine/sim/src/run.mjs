...
const result={
  version:"shared-core-e6-v1",runsPerPolicy:runs,seedBase,
  coverage:{
    ...
    passiveEffects:"Gear/Artifact subset plus Long Odds and tie-resolution rules through core EffectResolver",
    remaining:"Contraband policy, Brass Caliper, Stuck Key, Ash Ledger, Receipt From Nowhere, Small Change Purse; Seal-Whelp and Seal-Bearer manipulation reactions still use explicit policy adapters"
  },
...
if(args.output){
  ...
  const lines=[
    "# Shared-Core Simulation E6 V1","",`Runs per policy: **${runs}**`,"",
  ...
  for(const[policy,s]of Object.entries(result.policies)){
    lines.push(`| ${policy} | ${pct(s.clearRate)} | ${s.avgFinalHp.toFixed(2)} | ${s.avgDamageTaken.toFixed(2)} | ${s.avgRounds.toFixed(2)} | ${s.avgSpoils.toFixed(2)} | ${pct(s.rawBandFrequency[4])} | ${pct(s.rewardBandFrequency[4])} | ${s.avgElites.toFixed(2)} |`);
  }
