import {
  ...
  floor1NormalEnemies, floor1Elites, firstDoor, bossPhaseAsEnemy, floor1Events,
  ...
  generateLootDraft, generateShopStock, resolveEventChoice,
...
function manipulationOptions(state,run,uses,enemy){
  ...
  if((uses.bump??0)<bumpLimit) state.playerRoll.forEach((die)=>{if(!die.fixed)for(const delta of [-1,1]){const value=bump(die.value,delta);if(value!==die.value)out.push({state:updatePlayerDie(state,withValue(die,value)),uses:{...uses,bump:(uses.bump??0)+1}});}});
  if(owned(run,"mirror-shard")&&!uses.flip) state.playerRoll.forEach((die)=>{if(!die.fixed)out.push({state:updatePlayerDie(state,withValue(die,flip(die.value))),uses:{...uses,flip:1}});});
  if(owned(run,"carbon-paper")&&!uses.copy) for(let target=0;target<4;target+=1){if(state.playerRoll[target].fixed)continue;for(let source=0;source<4;source+=1){if(source!==target)out.push({state:updatePlayerDie(state,withValue(state.playerRoll[target],state.playerRoll[source].value)),uses:{...uses,copy:1}});}}
  if(owned(run,"blank-face")&&!uses.transmute) state.playerRoll.forEach((die)=>{if(!die.fixed)for(const value of [1,6])if(value!==die.value)out.push({state:updatePlayerDie(state,withValue(die,value)),uses:{...uses,transmute:1}});});
  const sealed=(enemy.ruleIds??[]).includes("rule:first-door:sealed");
  if(owned(run,"hidden-hand")&&!uses.interfere&&!sealed){const locked=[...state.enemyLocked];const high=Math.max(...locked);const i=locked.indexOf(high);if(high>1){locked[i]=bump(high,-1);out.push({state:{...state,enemyLocked:locked},uses:{...uses,interfere:1}});}}
  return out;
}

function optimizeManipulations(state,run,uses,enemy,effects,effectState){
  let current=state,currentUses=uses,currentBest=bestPair(state,effects,effectState,run),reactiveTriggered=false,manipulations=0;
  for(let step=0;step<4;step+=1){
    ...
    if(!winner)break;
    current=winner.state;currentUses=winner.uses;currentBest=winner.best;effects=winner.effects;manipulations+=1;reactiveTriggered=true;
  }
  return {state:current,uses:currentUses,best:currentBest,effects,manipulations};
}

function combat(run,baseEnemy,{boss=false}={}){
  let enemy=baseEnemy;let state=createCombatFromPlayerState({hp:run.progression.hp,maxHp:run.progression.maxHp},enemy);let effectState=createEffectState(),uses={},finalSpoils=null,rounds=0;
  while(state.player.hp>0&&state.enemy.hp>0&&rounds<40){
    rounds+=1;if(boss)enemy=bossPhaseAsEnemy(firstDoor,state.enemy.hp);
    ...
    const committed=commitWithEffects(state,optimized.best.ids,effects,effectState);state=committed.state;effectState=committed.effectState;
    ...
    if((enemy.ruleIds??[]).includes("rule:turnback:refusal")&&committed.preview.margin===0&&state.enemy.hp>0)state={...state,enemy:{...state.enemy,hp:Math.min(state.enemy.maxHp,state.enemy.hp+1)}};
    if(boss&&(enemy.ruleIds??[]).includes("rule:first-door:both-ways")&&committed.preview.margin===0)state={...state,player:{...state.player,hp:Math.max(0,state.player.hp-2)},enemy:{...state.enemy,hp:Math.max(0,state.enemy.hp-2)}};
    if(state.player.hp<=0||state.enemy.hp<=0)break;
  }
  run.progression={...run.progression,hp:state.player.hp};run.stats.rounds+=rounds;run.stats.damageTaken+=Math.max(0,run.stats.hpBeforeCombat-state.player.hp);
...
function rewardCombat(run,enemy,spoils){
  run.economy=addCoins(run.economy,enemy.coins);const xp=grantXp(run.progression,enemy.xp);run.progression=xp.state;if(xp.levelsGained.includes(3)&&!run.technique)run.technique=run.policy==="greedy"?"long-odds":"steady-hand";
  if(spoils!==null){const draft=generateLootDraft(spoils,floor1ItemRegistry,run.inventory,run.rng.stream("loot"),enemy.rewardBandUplift??0);run.stats.bands[draft.band]=(run.stats.bands[draft.band]??0)+1;run.stats.spoils.push(spoils);takeDraft(run,draft);}
}
...
function eventChoice(run,event){
  if(event.id.endsWith("unnumbered-door")){
    if(run.policy==="safe")return hpRatio(run)>.9?"knock":"leave";
    if(run.policy==="opportunist")return hpRatio(run)>=.8?"force":"knock";
    return run.progression.hp>3?"force":"knock";
  }
...
function visitEvent(run){
  run.stats.events+=1;const event=selectEvent(floor1Events,run.eventHistory,run.rng.stream("event:select"));run.eventHistory.push(event.id);const choice=eventChoice(run,event);const beforeHp=run.progression.hp;
...
function makeRun(seed,policy){return{seed,policy,rng:new RngService(seed),progression:createProgression(20),economy:createEconomy(0),inventory:createInventory({ARMOR:"work-apron"}),technique:null,history:[],eventHistory:[],stats:{rounds:0,damageTaken:0,hpBeforeCombat:20,bands:{},spoils:[],itemsTaken:{},shops:0,shopHeals:0,shopItems:0,draftSkips:0,events:0,elites:0}};}
...
export function simulateFloor(seed,policy="opportunist"){
  ...
  for(let row=0;row<thresholdsFloor.rows.length&&run.progression.hp>0;row+=1){
    ...
    if(node.type==="COMBAT"||node.type==="ELITE"){
      const pool=node.type==="ELITE"?floor1Elites:floor1NormalEnemies;const enemy=selectEncounter(pool,run.history,run.rng.stream("encounter"));run.history.push({enemyId:enemy.id,instinct:enemy.instinct});if(enemy.elite)run.stats.elites+=1;run.stats.hpBeforeCombat=run.progression.hp;const result=combat(run,enemy);if(!result.win)break;rewardCombat(run,enemy,result.spoils);
    }else if(node.type==="SHOP")visitShop(run);else if(node.type==="EVENT")visitEvent(run);
  }
  let clear=false;
  if(run.progression.hp>0){run.stats.hpBeforeCombat=run.progression.hp;const result=combat(run,bossPhaseAsEnemy(firstDoor,firstDoor.maxHp),{boss:true});clear=result.win;if(clear){run.economy=addCoins(run.economy,firstDoor.coins);run.progression=grantXp(run.progression,firstDoor.xp).state;run.progression={...run.progression,hp:Math.min(run.progression.maxHp,run.progression.hp+firstDoor.clearHeal)};}}
  return{seed,policy,clear,hp:run.progression.hp,maxHp:run.progression.maxHp,level:run.progression.level,xp:run.progression.xp,coins:run.economy.coins,rounds:run.stats.rounds,damageTaken:run.stats.damageTaken,bands:run.stats.bands,spoils:run.stats.spoils,shops:run.stats.shops,events:run.stats.events,elites:run.stats.elites,itemsTaken:run.stats.itemsTaken};
}

export function summarize(rows){
  const bands={1:0,2:0,3:0,4:0};const spoils=[];for(const row of rows){for(const[k,v]of Object.entries(row.bands))bands[k]+=v;spoils.push(...row.spoils);}const bandTotal=Object.values(bands).reduce((a,b)=>a+b,0)||1;const avg=(f)=>rows.reduce((s,r)=>s+f(r),0)/rows.length;
  return{runs:rows.length,clearRate:avg(r=>r.clear?1:0),avgFinalHp:avg(r=>r.hp),avgDamageTaken:avg(r=>r.damageTaken),avgRounds:avg(r=>r.rounds),avgLevel:avg(r=>r.level),avgCoins:avg(r=>r.coins),avgElites:avg(r=>r.elites),avgShops:avg(r=>r.shops),avgEvents:avg(r=>r.events),avgSpoils:spoils.length?spoils.reduce((a,b)=>a+b,0)/spoils.length:0,bandFrequency:Object.fromEntries(Object.entries(bands).map(([k,v])=>[k,v/bandTotal]))};
}
