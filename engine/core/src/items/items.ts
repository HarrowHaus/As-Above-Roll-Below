...
export function createInventory(startingGear: Partial<Record<GearSlot,string>> = {}): InventoryState {
  return { gear:{WEAPON:startingGear.WEAPON??null,ARMOR:startingGear.ARMOR??null,UTILITY:startingGear.UTILITY??null}, artifacts:[], contraband:[] };
}

export function ownedItemIds(inventory: InventoryState): string[] {
  return [...Object.values(inventory.gear).filter((id): id is string=>id!==null),...inventory.artifacts,...inventory.contraband];
}
...
export function removeItem(inventory:InventoryState,itemId:string):InventoryState {
  ...
  if(inventory.artifacts.includes(itemId)){
    const index=inventory.artifacts.indexOf(itemId); const artifacts=[...inventory.artifacts]; artifacts.splice(index,1); return {...inventory,artifacts};
  }
  if(inventory.contraband.includes(itemId)){
    const index=inventory.contraband.indexOf(itemId); const contraband=[...inventory.contraband]; contraband.splice(index,1); return {...inventory,contraband};
  }
...
export function applyTakeItem(inventory:InventoryState,item:ItemDefinition,registry:ItemRegistry,replacementItemId?:string):InventoryTakeResult {
  ...
  if(item.category==="GEAR"){
    const slot=item.gearSlot!; removedItemId=inventory.gear[slot]; next={...inventory,gear:{...inventory.gear,[slot]:item.id}};
  }else if(item.category==="ARTIFACT"){
    let artifacts=[...inventory.artifacts]; if(replacementItemId){removedItemId=replacementItemId;artifacts=artifacts.filter((id)=>id!==replacementItemId);} artifacts.push(item.id); next={...inventory,artifacts};
  }else{
    const contraband=[...inventory.contraband]; if(replacementItemId){removedItemId=replacementItemId;const index=contraband.indexOf(replacementItemId);if(index>=0)contraband.splice(index,1);} contraband.push(item.id); next={...inventory,contraband};
  }
