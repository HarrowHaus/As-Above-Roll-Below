...
export class ShopScene extends Scene {
  ...
  create():void {
    ...
    offers.forEach((offer,index)=>this.renderOffer(offer,180+index*235,315));
    this.button(1080,315,200,130,`FIELD DRESSING\nHEAL ${shop.healAmount} HP\n${shop.healPrice} COINS`,()=>this.buyHeal(),0x26302c,0x557b5d);
    this.button(1080,510,200,48,"LEAVE",()=>{clientRun.leaveShop();this.scene.start("MapScene");},0x232b31,0x66727a);
    this.refreshHeader();
  }
  ...
  private renderOffer(offer:ShopItemOffer,x:number,y:number):void {
    ...
    const action=this.add.text(0,108,"BUY",{fontFamily:"monospace",fontSize:"12px",color:"#e4ded2"}).setOrigin(0.5);
    const container=this.add.container(x,y,[bg,title,kind,price,action]);
    bg.on("pointerdown",()=>this.buy(offer));bg.on("pointerover",()=>{if(!this.sold.has(offer.itemId))bg.setStrokeStyle(3,0xd1aa59);});bg.on("pointerout",()=>bg.setStrokeStyle(2,0x46535d));
    this.cardObjects.set(offer.itemId,container);
  }

  private buy(offer:ShopItemOffer):void {
    if(this.sold.has(offer.itemId))return;
    ...
    if(plan.requiresReplacement){this.showReplacement(offer,plan.replacementCandidates);return;}
  ...
  private showReplacement(offer:ShopItemOffer,candidates:readonly string[]):void {
    const shade=this.add.rectangle(640,360,1280,720,0x050709,0.88).setInteractive();
    this.add.rectangle(640,360,650,350,0x151b20).setStrokeStyle(2,0x9a7740);
    this.add.text(640,235,"REPLACE INVENTORY",{fontFamily:"Georgia, serif",fontSize:"25px",color:"#eee6d5"}).setOrigin(0.5);
    candidates.forEach((id,index)=>{const item=floor1ItemRegistry[id]!;this.button(640,310+index*56,440,42,`REPLACE ${item.displayName.toUpperCase()}`,()=>{clientRun.buyShopItem(offer,id);this.markSold(offer.itemId);shade.destroy();this.scene.restart();});});
    this.button(640,500,180,40,"CANCEL",()=>this.scene.restart(),0x20262b,0x6c777e);
  }

  private buyHeal():void {
    const shop=clientRun.currentShop!;
    if(clientRun.state.progression.hp>=clientRun.state.progression.maxHp){this.statusText.setText("Already at full HP.");return;}
    if(clientRun.state.economy.coins<shop.healPrice){this.statusText.setText("Not enough Coins.");return;}
    clientRun.buyShopHealing();this.statusText.setText(`Healed ${shop.healAmount}.`);this.refreshHeader();
  }

  private markSold(id:string):void {this.sold.add(id);const card=this.cardObjects.get(id);if(card)card.setAlpha(0.32);}
