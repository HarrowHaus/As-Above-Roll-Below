...
export class CombatScene extends Scene {
  ...
  private commitSelection():void {
    ...
    const playerDamage=result.events.filter((event)=>event.type==="damage"&&event.target==="player").reduce((sum,event)=>sum+event.amount,0);const ledger=ashLedgerCoinAward(ownedItemIds(clientRun.state.inventory).includes("ash-ledger"),this.ashLedgerTriggered,playerDamage);this.ashLedgerTriggered=ledger.triggered;if(ledger.coins)clientRun.grantCoins(ledger.coins);
