...
class ClientRunSession {
  ...
  combatEffects(enemy:EnemyDefinition):readonly EffectDefinition[] {
    ...
    if(this.hasTechnique("long-odds"))sourceIds.add("technique:long-odds");
    const effects=Object.values(floor1EffectRegistry).filter((effect)=>sourceIds.has(effect.sourceId));for(const ruleId of enemy.ruleIds??[]){const effect=floor1EffectRegistry[ruleId];if(effect)effects.push(effect);}return effects;
  }
  ...
  finishCombatVictory(playerHp:number,finalSpoils:number|null):"LOOT"|"MAP"|"VICTORY" {
    ...
    let progression={...this.run.progression,hp:playerHp};const economy=addCoins(this.run.economy,this.currentEnemy.coins);progression=grantXp(progression,this.currentEnemy.xp).state;this.run=withRunResources(this.run,{progression,economy});
    if(this.run.phase==="BOSS"){
      this.run=withRunResources(this.run,{progression:heal(this.run.progression,firstDoor.clearHeal)});
      this.pendingLoot=generateBossDraft(floor1BossDraftItemIds,floor1ItemRegistry,this.run.inventory,this.rng.stream("boss:reward"));this.pendingLootKind="BOSS";return "LOOT";
    }
  ...
  takeLootOffer(offer:LootOffer,replacementItemId?:string):void {
    if(offer.type==="COINS")this.run=withRunResources(this.run,{economy:addCoins(this.run.economy,offer.amount)});else this.takeItem(offer.itemId,replacementItemId);
    this.pendingLoot=null;this.pendingLootKind=null;this.run=completeRunNode(this.run);
  }
  ...
  chooseTechnique(id:TechniqueId):void {
    const level=this.pendingTechniqueLevel;if(level===null)throw new Error("No pending Technique choice");
    const allowed=level===3?["steady-hand","long-odds"]:level===5?["clean-exit","overrule"]:[];
    if(!allowed.includes(id))throw new Error(`${id} is not available at Level ${level}`);
    this.techniques.push(id);this.run=withRunResources(this.run,{progression:resolveTechniqueChoice(this.run.progression,level)});
  }
  ...
  resolveEvent(choiceId:string,selectedItemId?:string):readonly PendingEventOffer[] {
    if(!this.currentEvent)throw new Error("No current Event");const resolved=resolveEventChoice(this.currentEvent,choiceId,{progression:this.run.progression,economy:this.run.economy,inventory:this.run.inventory},floor1ItemRegistry,this.rng.stream("event:outcome"),selectedItemId?{itemId:selectedItemId}:{});
    this.run=withRunResources(this.run,resolved.state);this.pendingEventOffers=[...resolved.itemOffers];this.revealNextRequested=this.revealNextRequested||resolved.revealNextEncounter;if(this.run.progression.hp<=0){this.run=defeatRun(this.run);return [];}return this.pendingEventOffers;
  }
